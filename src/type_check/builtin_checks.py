...
def sequence_check(seq: Sequence, *args: type, chain: Chain) -> bool:
    if len(args) == 0:
        ...
        for i, el in enumerate(seq):
            type_assert(el, typ, chain=chain.enter(Chain.Key(i)))
...
def set_check(s: set, typ: type, *, chain: Chain) -> bool:
    for el in s:
        type_assert(el, typ, chain=chain.enter(Chain.Key('?')))
...
def dict_check(d: dict, key_type: type, value_type: type=Any, *, chain: Chain) -> bool:
    for key, val in d.items():
        type_assert(key, key_type, chain=chain.enter("<key>"))
        type_assert(val, value_type, chain=chain.enter(Chain.Key(key)))
